...
def update_obstacle_memory(red_blocks, green_blocks):
    ...
    if red_blocks or green_blocks:
        ...
        if time_since_last_detection < OBSTACLE_MEMORY_TIMEOUT:
            ...
            logger.debug(
                f"Using remembered obstacles: {len(red_blocks)} red, {len(green_blocks)} green (age: {time_since_last_detection:.2f}s)")
        else:
            # Clear old memory
            last_seen_obstacles = {"red": [], "green": [], "timestamp": 0}
...
def calculate_steering(blocks, color="none"):
    ...
    if not blocks:
        if front_dist < 55 and left_dist < 90 < right_dist:
            steering_adjustment = SERVO_RIGHT - SERVO_STRAIGHT
            turn_type = f"TURN_RIGHT"
        elif front_dist < 55 and left_dist > 90 > right_dist:
            steering_adjustment = SERVO_LEFT - SERVO_STRAIGHT
            turn_type = f"TURN_LEFT"
    ...
    if color == "GREEN":
        ...
        if front_dist < 55 and left_dist < 90 < right_dist:
            steering_adjustment = SERVO_RIGHT
            turn_type = f"TURN_RIGHT"
        elif front_dist < 55 and left_dist > 90 > right_dist:
            steering_adjustment = SERVO_LEFT
            turn_type = f"TURN_LEFT"
    ...
    logger.debug(
        f"Steering calculation: {color} block in {region} at ({block_center_x},{y}) -> {turn_type} | area={block_area} | adjustment={steering_adjustment:.1f}")
    return steering_adjustment, turn_type
...
def decide_steering():
    ...
    if red_blocks or green_blocks:
        ...
        logger.debug(
            f"All detections: RED blocks={len(red_blocks)}{all_red_areas}, GREEN blocks={len(green_blocks)}{all_green_areas}")
    ...
    if closest_block is not None:
        ...
        print(
            f"No obstacles - Yaw: {yaw:.1f}°, Recovery: {recovery_steering}, YawCorr: {yaw_correction:.1f}, Total: {total_correction:.1f}")
        logger.debug(
            f"Recovery mode: yaw={yaw:.1f}°, recovery_steering={recovery_steering}, yaw_correction={yaw_correction:.1f}, total={total_correction:.1f}")
...
def draw_debug(frame, red_blocks, green_blocks, steering, state):
    ...
    # Draw status info
    cv2.putText(debug_frame, f"Steering: {steering:.1f}", (10, FRAME_H - 150), cv2.FONT_HERSHEY_SIMPLEX, 0.8,
                (255, 255, 0), 2)
    ...
    cv2.putText(debug_frame, f"Last Turn: {last_turn_direction}", (10, FRAME_H - 60), cv2.FONT_HERSHEY_SIMPLEX, 0.8,
                (255, 255, 0), 2)
    cv2.putText(debug_frame, f"Recovery: {recovery_mode}", (10, FRAME_H - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8,
                (255, 255, 0), 2)
...
def main():
    ...
    try:
        ...
        print(
            f"Regions: LEFT(0-{LEFT_REGION[1]}), CENTER({CENTER_REGION[0]}-{CENTER_REGION[1]}), RIGHT({RIGHT_REGION[0]}-{FRAME_W})")

        logger.info("Starting region-based navigation...")
        logger.info(
            f"Frame regions: LEFT(0-{LEFT_REGION[1]}), CENTER({CENTER_REGION[0]}-{CENTER_REGION[1]}), RIGHT({RIGHT_REGION[0]}-{FRAME_W})")
        logger.info(
            f"Turn intensity config: MIN={MIN_TURN}°, MAX={MAX_TURN}°, SMALL_THRESHOLD={SIZE_THRESHOLD_SMALL}, LARGE_THRESHOLD={SIZE_THRESHOLD_LARGE}")
        ...
        logger.info(
            f"Navigation session completed: {loop_count} loops in {total_runtime:.2f} seconds ({loop_count / total_runtime:.1f} Hz)")
